...
#ifndef _CMP_URINGIC_H_
#define _CMP_URINGIC_H_
...
namespace cmpex {
  ...
  namespace cmp {
    ...
    //======================================================================
    // This class represents a unidirectional ring interconnect for CMP.
    ...
    // Every ring router has 2 physical (3 virtual) input ports:
    //   Index 0: component
    //   Index 1, 2: pass-through connection
    // Every ring router has 2 output ports:
    //   Index 0: component
    //   Index 1: pass-through
    //======================================================================

    class URingIc : public Interconnect {
    ...
    private:
      ...
      URingIc ( const URingIc& );

      URingIc& operator = ( const URingIc& );
    ...
    private:
      ...
      // Matrix of traffic rates per IO-port pair per router.
      ...
      // Matrix of buffer delays per I-port per router.
      // To access the delay value for input port I of router R,
    ...
    int URingIc::CompCnt () const {
      return compCnt_;
    }

    void URingIc::CompCnt (int c) {
      compCnt_ = c;
      totalCompCnt_ = c+1; // add iface component
    }

    int URingIc::TotalCompCnt () const {
      return totalCompCnt_;
    }

    int URingIc::RouterDelay () const {
      return routerDelay_;
    }

    void URingIc::RouterDelay (int d) {
      routerDelay_ = d;
    }
    ...
    double& URingIc::BufDelay (UShort subnIdx, UShort rIdx, UShort iPort) {
