...
#ifndef _CMP_XBARIC_H_
#define _CMP_XBARIC_H_
...
namespace cmpex {
  ...
  namespace cmp {
    ...
    //======================================================================
    // This class represents a crossbar interconnect for CMP.
    // A crossbar has a list of device components and possibly memory controllers
    ...
    class XBarIc : public Interconnect {
    ...
    private:
      ...
      XBarIc& operator = ( const XBarIc& );

      // -------------------------- Attributes -----------------------------

    private:

      int compCnt_; // number of components in crossbar
      
      int totalCompCnt_; // total number of components in crossbar, with iface and mcs
      
      int delay_; // crossbar delay (in cycles)
    ...
    int XBarIc::CompCnt () const {
      return compCnt_;
    }

    void XBarIc::CompCnt (int c) {
      compCnt_ = c;
      totalCompCnt_ = c+1; // add iface component
    }

    int XBarIc::TotalCompCnt () const {
      return totalCompCnt_;
    }

    int XBarIc::Delay () const {
      return delay_;
    }
    ...
    double& XBarIc::BufDelay (UShort subnIdx, UShort iPort) {
