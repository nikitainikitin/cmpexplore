...
if (@ARGV == 6) {
	...
	foreach (@AoA_flp){
		...
		if ($w>$tot_x/$smallest_shown && $h>$tot_y/$smallest_shown){
			...
			'" fill="black" text_anchor="start" style="font-size:36" > '. @{$_}[0] .' </text>'."\n";
		}
	}
	}
...
#			$level=int(($max_t-(@{$_}[5]))/($max_t-$min_t)*($num_levels-1));
			...
			if ($i%3==0) {
				...
				'" fill="black" text_anchor="start" style="font-size:400" > '. $scale_value .' </text>'."\n";
			}
		}
		$clr_ymin+=$h2;
	}
	my $min_t_celsius;
	$min_t_celsius=$min_t-273.15;
	$min_t_celsius=~s/^(\d+)\.(\d)(\d)\d+/$1\.$2$3/;
	$fp.="\t".'<text x="'.$scale_xmin .'" y="'. $clr_ymin .
	'" fill="black" text_anchor="start" style="font-size:400" > '. $min_t_celsius .' </text>'."\n";
	}
